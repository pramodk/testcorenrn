...
for i in $names ; do
  mpirun -n 2 ./x86_64/special -mpi -c sim_time=100 test${i}.hoc
...
for i in $names ; do
  if [ "${i}" = "patstim" ]
  then
          mpirun -n 2 ./x86_64/special-core -mpi -d test${i}dat -e 100 --pattern patstim.spk
  else
          mpirun -n 2 ./x86_64/special-core -mpi -d test${i}dat -e 100
  fi
